...
private:
    ...
    void update_saved_target(const cv::Mat &image, const STrack &track)
    {
        ...
        // 每2秒更新一次特征
        if ((current_time - tracked_target_.last_update_time).seconds() >= 2.0)
        {
            tracked_target_.bbox = cv::Rect(
                static_cast<int>(track.tlbr[0]),
                static_cast<int>(track.tlbr[1]),
    ...
    bool isHandsUp(const STrack &track)
    {
        ...
        std::vector<CachedKeypoint> cached_keypoints(track.keypoints.size());
        for (size_t i = 0; i < track.keypoints.size(); i++)
        {
            cached_keypoints[i].x = track.keypoints[i].x;
            cached_keypoints[i].y = track.keypoints[i].y;
            cached_keypoints[i].score = track.keypoints_score[i];
            cached_keypoints[i].valid = (cached_keypoints[i].x > 0 &&
                                         cached_keypoints[i].y > 0 &&
                                         cached_keypoints[i].score >= kpt_conf_threshold_);
        }

        // 检查必须的关键点是否有效
        if (!cached_keypoints[LEFT_SHOULDER].valid ||
            !cached_keypoints[RIGHT_SHOULDER].valid ||
            !cached_keypoints[NOSE].valid)
        {
            return false;
        }

        // 使用整数运算进行比较（更快）
        int left_shoulder_y = static_cast<int>(cached_keypoints[LEFT_SHOULDER].y);
        int right_shoulder_y = static_cast<int>(cached_keypoints[RIGHT_SHOULDER].y);
        int nose_y = static_cast<int>(cached_keypoints[NOSE].y);

        bool left_hand_up = false;
        if (cached_keypoints[LEFT_WRIST].valid && cached_keypoints[LEFT_ELBOW].valid)
        {
            int left_wrist_y = static_cast<int>(cached_keypoints[LEFT_WRIST].y);
            int left_wrist_x = static_cast<int>(cached_keypoints[LEFT_WRIST].x);
            int left_shoulder_x = static_cast<int>(cached_keypoints[LEFT_SHOULDER].x);

            left_hand_up = (left_wrist_y < left_shoulder_y) &&
                           (left_wrist_y < nose_y) &&
                           (std::abs(left_wrist_x - left_shoulder_x) < 100);
        }

        bool right_hand_up = false;
        if (cached_keypoints[RIGHT_WRIST].valid && cached_keypoints[RIGHT_ELBOW].valid)
        {
            int right_wrist_y = static_cast<int>(cached_keypoints[RIGHT_WRIST].y);
            int right_wrist_x = static_cast<int>(cached_keypoints[RIGHT_WRIST].x);
            int right_shoulder_x = static_cast<int>(cached_keypoints[RIGHT_SHOULDER].x);

            right_hand_up = (right_wrist_y < right_shoulder_y) &&
                            (right_wrist_y < nose_y) &&
                            (std::abs(right_wrist_x - right_shoulder_x) < 100);
        }
