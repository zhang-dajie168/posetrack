...
{
	track_thresh = 0.2; // 目标激活的置信度阈值
	high_thresh = 0.6;	// 高分检测的阈值
	match_thresh = 0.7; // 匹配的阈值*（IOU或者外观相似度）
...
std::vector<STrack> BYTETracker::update(const std::vector<Object> &objects)
{
	...
	if (objects.size() > 0)
	{
		for (int i = 0; i < objects.size(); i++) // object.box=[x1,y1,width,height]
		{
			...
			// 添加关键点信息
			if (!objects[i].keypoints.empty())
			{
				strack.set_keypoints(objects[i].keypoints, objects[i].keypoints_score);
			}
