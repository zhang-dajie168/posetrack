...
bool YOLOv11Pose::init_model()
{
    ...
    RDK_CHECK_SUCCESS(
        hbDNNInitializeFromFiles(&ctx_.packed_dnn_handle, &model_file_name, 1),
    ...
    int model_count = 0;
    RDK_CHECK_SUCCESS(
        hbDNNGetModelNameList(&model_name_list, &model_count, ctx_.packed_dnn_handle),
        "hbDNNGetModelNameList failed");

    const char *model_name = model_name_list[0];
    RDK_CHECK_SUCCESS(
        hbDNNGetModelHandle(&ctx_.dnn_handle, ctx_.packed_dnn_handle, model_name),
        "hbDNNGetModelHandle failed");

    // 3. 获取输入属性
    RDK_CHECK_SUCCESS(
        hbDNNGetInputTensorProperties(&ctx_.input_properties, ctx_.dnn_handle, 0),
    ...
    // 获取输出数量
    RDK_CHECK_SUCCESS(
        hbDNNGetOutputCount(&output_count_, ctx_.dnn_handle),
        "hbDNNGetOutputCount failed");

    // std::cout << "\033[31m Model initialization time = " << std::fixed << std::setprecision(2)
    //           << std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now() - init_start).count() / 1000.0
    //           << " ms\033[0m" << std::endl;

    return true;
}

bool YOLOv11Pose::preprocess_image(const cv::Mat &src, cv::Mat &dst,
                                   ...
                                   int &x_shift, int &y_shift)
{
    ...
    { // LetterBox
        ...
        cv::copyMakeBorder(dst, dst,
                           y_shift, ctx_.input_H - new_h - y_shift,
                           x_shift, ctx_.input_W - new_w - x_shift,
    ...
    //           << std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now() - preprocess_start).count() / 1000.0
    //           << " ms\033[0m" << std::endl;
...
bool YOLOv11Pose::run_inference(const cv::Mat &processed_img)
{
    ...
    memcpy(input_tensor_.sysMem[0].virAddr, processed_img.data,
           processed_img.total() * processed_img.elemSize());
    ...
    for (int i = 0; i < output_count_; i++)
    {
        hbDNNTensorProperties output_properties;
        RDK_CHECK_SUCCESS(
            hbDNNGetOutputTensorProperties(&output_properties, ctx_.dnn_handle, i),
    ...
    RDK_CHECK_SUCCESS(
        hbDNNInfer(&task_handle, &output_tensors_, &input_tensor_, ctx_.dnn_handle, &infer_ctrl_param),
        "hbDNNInfer failed");
    ...
    //           << std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now() - inference_start).count() / 1000.0
    //           << " ms\033[0m" << std::endl;
    return true;
}

void YOLOv11Pose::process_scale_output(hbDNNTensor *output,
    ...
    for (int h = 0; h < grid_h; h++)
    {
        for (int w = 0; w < grid_w; w++)
        {
            ...
            for (int j = 0; j < KPT_NUM; j++)
            {
                ...
                kpt_score[j] = 1.0f / (1.0f + std::exp(-cur_kpts_raw[KPT_ENCODE * j + 2]));
            }
...
bool YOLOv11Pose::postprocess(float conf_thres_raw, float kpt_conf_thres_raw)
{
    ...
    // 处理三个尺度的输出
    process_scale_output(output_tensors_, ctx_.order[0], ctx_.order[1], ctx_.order[6],
                         ctx_.H_8, ctx_.W_8, 8, conf_thres_raw, kpt_conf_thres_raw);

    process_scale_output(output_tensors_, ctx_.order[2], ctx_.order[3], ctx_.order[7],
                         ctx_.H_16, ctx_.W_16, 16, conf_thres_raw, kpt_conf_thres_raw);

    process_scale_output(output_tensors_, ctx_.order[4], ctx_.order[5], ctx_.order[8],
                         ctx_.H_32, ctx_.W_32, 32, conf_thres_raw, kpt_conf_thres_raw);
    ...
    //           << std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now() - postprocess_start).count() / 1000.0
    //           << " ms\033[0m" << std::endl;
...
void YOLOv11Pose::render_results(cv::Mat &img, const std::vector<int> &indices)
{
    ...
    for (auto idx : indices)
    {
        ...
        cv::putText(img, text, cv::Point(x1, y1 - 5),
                    ...
                    cv::Scalar(0, 0, 255), 1.0, cv::LINE_AA);
    ...
    //           << std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now() - render_start).count() / 1000.0
    //           << " ms\033[0m" << std::endl;
}
...
void YOLOv11Pose::release_resources()
{
    ...
    if (ctx_.dnn_handle)
    {
        hbDNNRelease(ctx_.dnn_handle);
        ctx_.dnn_handle = nullptr;
    }
