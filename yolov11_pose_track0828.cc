/*
20250828 
举手两秒开始检测，冷却时间10秒，id丢失通过osnet找回
发布box和关键点，可视化，目标位置
*/
...
public:
                        ...
                        last_hands_up_detection_time_(rclcpp::Time(0, 0, RCL_ROS_TIME))
    {
        ...
        this->declare_parameter<int>("max_processing_fps", 15);
        this->declare_parameter<float>("reid_similarity_threshold", 0.6f);
        this->declare_parameter<float>("hands_up_duration_threshold", 2.0f);
...
private:
    ...
    void clear_tracked_target()
    {
        ...
        tracked_target_.reid_features.clear();
    ...
    // 更新已保存目标的信息（每秒更新一次）
    void update_saved_target(const cv::Mat &image, const STrack &track)
    {
        if (!osnet_ || tracked_target_.track_id == -1)
            return;

        auto current_time = this->now();

        // 每秒更新一次特征
        if ((current_time - tracked_target_.last_update_time).seconds() >= 1.0)
        {
            tracked_target_.bbox = cv::Rect(
                static_cast<int>(track.tlbr[0]),
                static_cast<int>(track.tlbr[1]),
                static_cast<int>(track.tlbr[2] - track.tlbr[0]),
                static_cast<int>(track.tlbr[3] - track.tlbr[1]));
            tracked_target_.last_update_time = current_time;
            ...
            try
            {
                ...
                {
                    tracked_target_.reid_features = osnet_->extract_features(target_roi);
                    RCLCPP_DEBUG(this->get_logger(), "Updated target %d features", tracked_target_.track_id);
                }
            }
            catch (const std::exception &e)
            {
                RCLCPP_WARN(this->get_logger(), "Failed to update ReID features: %s", e.what());
            }
        }
    }
    ...
    int reidentify_target(const cv::Mat &image, const std::vector<STrack> &tracks)
    {
        ...
        for (const auto &track : tracks)
        {
            try
            {
                ...
                RCLCPP_DEBUG(this->get_logger(), "ReID similarity with track %d: %.3f", track.track_id, similarity);
            ...
            {
                RCLCPP_WARN(this->get_logger(), "ReID feature extraction failed: %s", e.what());
            }
        }

        if (best_match_id != -1)
        {
            RCLCPP_INFO(this->get_logger(), "ReID match found: target %d -> track %d, similarity: %.3f",
    ...
    void image_callback(const sensor_msgs::msg::Image::ConstSharedPtr msg)
    {
        ...
        // 频率控制
        auto now = this->now();
        if ((now - last_process_time_).seconds() < min_process_interval_)
        {
            return;
        }
        ...
        try
        {
            ...
            if (detect_pose_pub_->get_subscription_count() > 0)
            {
    ...
    // void print_performance_stats(const ProcessingTimes &times)
    // {
    //     RCLCPP_INFO(this->get_logger(), "=== Performance Statistics (Frame %d) ===", total_frames_);
    ...
    void process_target_tracking(const cv::Mat &image, const std::vector<STrack> &tracks)
    {
        ...
        // 检查冷却时间：如果距离上次举手检测时间太短，忽略新的举手
        if ((current_time - last_hands_up_detection_time_).seconds() < hands_up_cooldown_)
        {
            return;
        }

        // 如果没有保存的目标，寻找符合条件的举手目标
                ...
                if (isHandsUp(track))
                {
                    ...
                    if (tracked_persons_.find(track_id) != tracked_persons_.end())
                    {
                        ...
                        if (!person.hands_up)
                        {
                            ...
                            person.hands_up = true;
                            RCLCPP_DEBUG(this->get_logger(), "Track %d started hands up at %.3f",
                                         track_id, current_time.seconds());
                        }

                        // 检查举手持续时间是否达到阈值
                        double hands_up_duration = (current_time - person.hands_up_start_time).seconds();
                        if (hands_up_duration >= hands_up_duration_threshold_ && !person.hands_up_confirmed)
                        {
                            person.hands_up_confirmed = true;
                            save_hands_up_target(image, track);
                            last_hands_up_detection_time_ = current_time;
                            RCLCPP_INFO(this->get_logger(), "Target %d confirmed hands up for %.1f seconds",
                                        track_id, hands_up_duration);
                            break;
                        }
                    }
                }
        ...
        const STrack *current_target = nullptr;
        ...
        for (const auto &track : tracks)
        {
            if (track.track_id == tracked_target_.track_id)
            {
                ...
                if (isHandsUp(track))
                {
                    auto &person = tracked_persons_[track.track_id];
                    ...
                    // 检查举手持续时间是否达到阈值
                    double hands_up_duration = (current_time - person.hands_up_start_time).seconds();
                    if (hands_up_duration >= hands_up_duration_threshold_)
                    {
                        RCLCPP_INFO(this->get_logger(), "Target %d raised hands again for %.1f seconds, clearing tracking",
                                    track.track_id, hands_up_duration);
        ...
        if (target_found && current_target)
        {
            // 更新目标信息
            update_saved_target(image, *current_target);
        }
        else
        {
            // 目标丢失，尝试重新识别
            RCLCPP_INFO(this->get_logger(), "Target %d lost, attempting re-identification", tracked_target_.track_id);
            ...
            if (reidentified_id != -1)
            {
                ...
                for (const auto &track : tracks)
                {
                    if (track.track_id == reidentified_id)
                    {
                        ...
                        update_saved_target(image, track);
                        RCLCPP_INFO(this->get_logger(), "Target reidentified: %d", reidentified_id);
                        break;
                    }
                }
            }
            ...
            {
                RCLCPP_INFO(this->get_logger(), "Target %d not found in current frame, keeping previous features",
    ...
    bool isHandsUp(const STrack &track)
    {
        ...
        // 检查关键点是否存在且坐标不为0
        auto has_valid_keypoint = [&](int index)
        {
            return index < track.keypoints.size() &&
                   track.keypoints[index].x > 0 &&
                   track.keypoints[index].y > 0 &&
                   track.keypoints_score[index] >= kpt_conf_threshold_;
        };

        // 必须的关键点
        if (!has_valid_keypoint(LEFT_SHOULDER) || !has_valid_keypoint(RIGHT_SHOULDER) ||
            !has_valid_keypoint(NOSE))
        {
            return false;
        }

        // 获取关键点位置
        cv::Point left_wrist, right_wrist, left_elbow, right_elbow;
        ...
        if (has_valid_keypoint(LEFT_WRIST) && has_valid_keypoint(LEFT_ELBOW))
        {
            left_wrist = cv::Point(track.keypoints[LEFT_WRIST].x, track.keypoints[LEFT_WRIST].y);
            left_elbow = cv::Point(track.keypoints[LEFT_ELBOW].x, track.keypoints[LEFT_ELBOW].y);

            // 多重条件判断：手腕高于肩膀，肘部高于肩膀，手腕高于鼻子
            left_hand_up = (left_wrist.y < left_shoulder.y) &&
                           (left_wrist.y < nose.y) &&
                           (std::abs(left_wrist.x - left_shoulder.x) < 100); // 避免误判侧向动作
        }

        // 检查右手举手
        bool right_hand_up = false;
        if (has_valid_keypoint(RIGHT_WRIST) && has_valid_keypoint(RIGHT_ELBOW))
        {
            right_wrist = cv::Point(track.keypoints[RIGHT_WRIST].x, track.keypoints[RIGHT_WRIST].y);
            right_elbow = cv::Point(track.keypoints[RIGHT_ELBOW].x, track.keypoints[RIGHT_ELBOW].y);
    ...
    void visualize_results(cv::Mat &img, const std::vector<STrack> &tracks)
    {
        ...
        static const cv::Scalar inactive_color(0, 0, 255);
        static const cv::Scalar keypoint_color(0, 0, 255);
        static const cv::Scalar skeleton_color(0, 255, 255);
        ...
        for (const auto &track : tracks)
        {
            ...
            cv::putText(img, label, cv::Point(bbox.x, bbox.y - 10),
                        cv::FONT_HERSHEY_SIMPLEX, 0.5, bbox_color, 2);

            // 绘制关键点 - 只绘制坐标不为0的点
            for (size_t i = 0; i < track.keypoints.size(); i++)
            {
                if (track.keypoints[i].x > 0 && track.keypoints[i].y > 0 &&
                    track.keypoints_score[i] >= kpt_conf_threshold_)
                {
                    cv::circle(img, cv::Point(track.keypoints[i].x, track.keypoints[i].y),
                               3, keypoint_color, -1);
                }
            }
            ...
            for (int i = 0; i < 19; i++)
            {
                int idx1 = skeleton_[i * 2];
                int idx2 = skeleton_[i * 2 + 1];

                if (idx1 < track.keypoints.size() && idx2 < track.keypoints.size() &&
                    track.keypoints[idx1].x > 0 && track.keypoints[idx1].y > 0 &&
                    track.keypoints[idx2].x > 0 && track.keypoints[idx2].y > 0 &&
                    track.keypoints_score[idx1] >= kpt_conf_threshold_ &&
                    track.keypoints_score[idx2] >= kpt_conf_threshold_)
                {
                    cv::line(img,
                             cv::Point(track.keypoints[idx1].x, track.keypoints[idx1].y),
                             cv::Point(track.keypoints[idx2].x, track.keypoints[idx2].y),
                             skeleton_color, 2);
                }
            }
        }
        ...
        if (tracked_target_.track_id != -1)
        {
            status += "Target ID " + std::to_string(tracked_target_.track_id);
            double cooldown_remaining = hands_up_cooldown_ - (this->now() - last_hands_up_detection_time_).seconds();
            if (cooldown_remaining > 0)
            {
                status += " | Cooldown: " + std::to_string(static_cast<int>(cooldown_remaining)) + "s";
            }
        }
        else
        {
    ...
    void print_tracking_info(const std::vector<STrack> &tracks)
    {

        RCLCPP_INFO(this->get_logger(), "===== Tracking Results =====");
        ...
        for (const auto &track : tracks)
        {
            ...
            // 检查是否在跟踪人员列表中
            auto it = tracked_persons_.find(track.track_id);
            bool is_tracked_person = (it != tracked_persons_.end());

            RCLCPP_INFO(this->get_logger(),
                        "Track ID: %d | Target: %s | Tracked: %s ",
                        track.track_id,
                        is_target_tracked ? "YES" : "NO",
                        is_tracked_person ? "YES" : "NO");
        }
