...
public:
    Yolov11PoseNode() : Node("yolov11_pose_node"),
                        ...
                        tracking_start_time_(rclcpp::Time(0, 0, RCL_ROS_TIME)),
                        cooldown_start_time_(rclcpp::Time(0, 0, RCL_ROS_TIME))
    {
...
private:
    ...
    int reidentify_target(const cv::Mat &image, const std::vector<STrack> &tracks)
    {
        ...
        RCLCPP_INFO(this->get_logger(), "开始ReID匹配，当前保存目标ID: %d", tracked_target_.track_id);
        ...
        for (const auto &track : tracks)
        {
            try
            {
                ...
                {
                    RCLCPP_WARN(this->get_logger(), "Track %d: 裁剪区域为空", track.track_id);
                    continue;
                }
                ...
                {
                    RCLCPP_WARN(this->get_logger(), "Track %d: 特征提取失败", track.track_id);
                    continue;
                }
                ...
                RCLCPP_INFO(this->get_logger(), "Track %d 与目标 %d 的相似度: %.3f (阈值: %.3f)",
                ...
                if (similarity > best_similarity)
                {
                    ...
                    RCLCPP_INFO(this->get_logger(), "新的最佳匹配: track %d, 相似度: %.3f", best_match_id, best_similarity);
                }
            }
            catch (const std::exception &e)
            {
                RCLCPP_WARN(this->get_logger(), "Track %d ReID特征提取失败: %s", track.track_id, e.what());
            }
        }

        if (best_match_id != -1)
        {
            RCLCPP_INFO(this->get_logger(), "ReID匹配成功: 目标 %d -> track %d, 相似度: %.3f",
                        tracked_target_.track_id, best_match_id, best_similarity);
        }
        else
        {
            RCLCPP_INFO(this->get_logger(), "ReID无匹配，最高相似度: %.3f (需要 > %.3f)",
                        best_similarity, reid_similarity_threshold_);
        }
    ...
    void visualize_results(cv::Mat &img, const std::vector<STrack> &tracks)
    {
        ...
        {
        case TrackingState::IDLE:
            status = "State: IDLE - Waiting for hands up";
            break;
        case TrackingState::TRACKING:
            status = "State: TRACKING - Target ID " + std::to_string(tracked_target_.track_id);
            if (tracked_target_.track_id != -1)
            {
                double tracking_time = (this->now() - tracking_start_time_).seconds();
                status += " | Time: " + std::to_string(static_cast<int>(tracking_time)) + "s";
            }
            break;
        case TrackingState::COOLDOWN:
            ...
            {
                status += " | Remaining: " + std::to_string(static_cast<int>(cooldown_remaining)) + "s";
            }
            break;
        }
